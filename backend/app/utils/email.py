...
def send_verification_email(to_email: str, code: str, purpose: str = "注册") -> bool:
    ...
    try:
        ...
        # Never log the code itself — only who was mailed and the SES message id,
        # so a delivery investigation can be run from CloudWatch alone.
        logger.info(
            "Verification email (%s) sent to %s via SES (MessageId=%s)",
            purpose,
            to_email,
        ...
        logger.error(
            "SES rejected verification email (%s) to %s: %s — %s",
            purpose,
            to_email,
            error.get("Code"),
            error.get("Message"),
        ...
        logger.error("Failed to send verification email (%s) to %s: %s", purpose, to_email, e)
        return False
...
def send_notification_email(to_email: str, sender_display_name: str) -> bool:
    ...
    inbox_url = "https://gradchoice.org/inbox"
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <h2 style="color: #0d9488; margin-bottom: 8px;">研选 GradChoice</h2>
        ...
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;">
        <p style="color: #9ca3af; font-size: 11px;">如不希望接收此类通知，请前往 <a href="https://gradchoice.org/profile" style="color: #0d9488;">个人主页</a> 关闭邮件通知。</p>
        <p style="color: #9ca3af; font-size: 11px;">研选 GradChoice — 帮助研究生选择导师</p>
    </div>
    """
    ...
    try:
        ...
        logger.info(
            "Notification email sent to %s via SES (MessageId=%s)",
            to_email,
            response.get("MessageId"),
        )
        return True
        ...
        error = e.response.get("Error", {})
        logger.error(
            "SES rejected notification email to %s: %s — %s",
            to_email,
            error.get("Code"),
            error.get("Message"),
        )
